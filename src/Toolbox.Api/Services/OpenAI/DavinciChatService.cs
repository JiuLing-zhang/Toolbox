...
public class DavinciChatService : IChatService
{
    private readonly string _apiAddress = "https://api.openai.com/v1/completions";
    private readonly string _apiKey;
    private readonly IHttpClientFactory _httpClientFactory;

    private readonly DavinciModel _model;
    public DavinciChatService(DavinciModel model, IHttpClientFactory httpClientFactory, IOptions<AppSettings> appSettingsOptions)
    {
        _model = model;
    ...
    public async Task<ApiResponse<string>> GetMessageAsync()
    {
        ...
        try
        {
            ...
            var result = JsonSerializer.Deserialize<DavinciResult>(responseBody);
            if (result == null)
            {
            ...
            return new ApiResponse<string>(0, "操作成功", result.Choices?[0].Text);
        }
        catch (Exception ex)
        {
    ...
    public async IAsyncEnumerable<string> GetStreamingMessageAsync()
    {
        ...
        while (!reader.EndOfStream)
        {
            ...
            var streamResult = JsonSerializer.Deserialize<DavinciResult>(currentLine);
            var data = streamResult?.Choices?[0].Text;
            if (data == null)
            {
                yield return $"01 ";
                continue;
